...
export default function Paginado({recipes,recipesPage,paginado}){
    ...
    for (let i = 0; i < Math.ceil(recipes/recipesPage); i++) {    // hacemos la logica y pusheamos el resultado para que nos de el numero de pagian 
            ...
            <ul className='paginado'>
                {numbers&&numbers.map((num)=>(
                        ...
                        <li className='num' key={num}>
                         <button className='scrolldown' onClick={()=>paginado(num)}>{num}</button>
                        </li>
